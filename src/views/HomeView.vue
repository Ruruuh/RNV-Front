...
function getRoles(ticket: Ticket) {
    const rolesObj = {
        roles: ["director", "hsu", "hr", "sdas", "finance", "none"],
        rolesTitleCase: ["Director", "HSU", "HR", "SDAS", "Finance"]
    }
    const ccsRolesObj = {
        roles: ["director", "sdirector", "hsu", "hr", "sdas", "finance", "none"],
        rolesTitleCase: ["Director", "Senior Director", "HSU", "HR", "SDAS", "Finance"]
    }
    if (ticket.department === 'CS' || ticket.department === 'IT') {
        return rolesObj
    } else {
        return ccsRolesObj
    }
...
const approvedBy = computed(() => {
    if (ticketToShow.value) {
        const rolesObj = getRoles(ticketToShow.value)
        const actionByIndex = rolesObj.roles.indexOf(ticketToShow.value.actionBy)
        const approvedByIndex = actionByIndex - 1

        if (approvedByIndex !== -1) {
            return rolesObj.rolesTitleCase[approvedByIndex]
        } else {
            return "No one yet."
        }
    }
...
const toApprove = computed(() => {
    if (ticketToShow.value) {
        const rolesObj = getRoles(ticketToShow.value)
        const actionByIndex = rolesObj.roles.indexOf(ticketToShow.value.actionBy)
        if (actionByIndex !== rolesObj.roles.length - 1) {
...
<template>
    <section class="home">
        ...
        <div v-if="role === 'user'" class="home__copy">
            <div><strong>Pending tickets</strong> - See a list of all your remaining tickets as well as its current
                status.</div>
            <div><strong>Completed tickets</strong> - See a list of all the tickets that had been approved/rejected.
            </div>
            <div><strong>Create ticket</strong> - Create a ticket to be reviewed by the Director, HSU, HR, SDAS, and
                Finance.</div>
            <div>Using the system means that you agree to our policies and guidelines. Click <strong><a
                        class="home__policy" @click="updateIsModalShown">here</a></strong> to review.</div>
        </div>
        <div v-else class="home__copy">
            ...
            <div v-if="ticketToShow" class="home__ticket">
                ...
                <base-progress :actionBy="ticketToShow.actionBy" :department="ticketToShow.department"></base-progress>
            </div>
            ...
            <div>For a refresher on our policies and guidelines, please click <strong><a class="home__policy"
                        @click="updateIsModalShown">here</a></strong>.</div>
        </div>
        ...
        <Teleport to="#app">
            <base-modal class="modal" v-if="isModalShown">
                <div class="modal__content">
                    ...
                    <div>1. Medicines shall be prescribed by a Health Services Unit (HSU) Physician. For medicines
                        prescribed by other physicians, the medical prescription shall be submitted to the HSU physician
                        for proper validation and entry in the medical referral form.</div>
                    <div>2. Maintenance medicines shall be prescribed for 30 days.</div>
                    <div>3. Vitamins may be prescribed if required as a care for a diagnosed illness, quantity should be
                        good for 30 days.</div>
                    <div>4. Immunizations may be availed through the medicine allowance provided that the amount of the
                        immunization package is covered by the medicine allowance.</div>
                    ...
                    <div class="modal__subcopy">
                        ...
                        <div>c. Injury sustained in bars, gambling houses or disreputable places or in participating in
                            the commission of a crime; </div>
                        <div>d. Sexually transmitted diseases;</div>
                        <div>e. Functional disorder of the mind;</div>
                        <div>f. Prosthetic work and appliances needed for the procedures such as brace, metal implants
                            and the amount of the stents;</div>
                        <div>g. Cosmetic purposes including dermatological for purposes not intended for curative
                            purposes;</div>
                        <div>h. All other cases of similar nature </div>
                    </div>
                    <div>6. Reimbursements for medicines during confinement should be covered by an approved Medicine
                        Referral Form (MRF) and the claim should be covered by official statement of expenses and
                        official receipt.</div>
                    <div>7. All MRFs shall be subject to approval of the SDAS.</div>
                    <div>8. Medicine reimbursements should be covered by an official receipt and prescription.
                        Procedures for reimbursements shall follow our procedure in requesting for Petty Cash/Check
                        Requests. Official receipts shall be recommended for payment by the SDAS.</div>
                    <div>9. The services provided and the medicines prescribed shall be for the treatment of a diagnosed
                        illness or injury and for the benefit solely of the associate.</div>
                </div>
            </base-modal>
        </Teleport>
    </section>
