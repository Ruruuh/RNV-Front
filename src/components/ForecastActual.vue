    ...
    <div class="forecast__actual">
        ...
        <div class="forecast__explanation" v-if="isExplanationShown">
            ...
            <div class="forecast__explanation-copy">The graph shows the projected number of demand between {{
                    actualStart?.toString().slice(5, actualStart.toString().length)
            }} and {{ actualEnd?.toString().slice(5,
        actualEnd.toString().length)
}}. You can take a point on the line graph and you would get the number of
                projected demand on the y-axis at a given date on the x-axis. You can see that there is not much
                variation in the projected demand between the two dates. This could be due to the lack of training data
                present within the system. You can use the test forecast above to check if the model could perform as
                intended given a decent amount of training data.</div>
        </div>
    </div>
</template>
