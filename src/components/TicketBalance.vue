...
const isEditBalanceShown = ref(false)
...
const amount = computed(() => {
    if (isEditBalanceShown.value) {
        if (currentBalance.value) {
            return currentBalance.value - approvedReimbursementTotal.value
        }
    } else if (!ticket.value?.creatorInfo.balance) {
        if (currentBalance.value) {
            return currentBalance.value - approvedReimbursementTotal.value
        }
    } else {
...
function updateIsEditBalanceShown() {
    isEditBalanceShown.value = !isEditBalanceShown.value
}
...
<template>
    ...
    <div class="balance" v-if="ticket?.balance">
        <div class="balance__title">Medicine Balance Info</div>
    ...
    <div class="balance" v-if="role === 'hr' && overview === 'pending'">
        <div class="balance__header">
            <div class="balance__title">Medicine Balance Info</div>
            <base-button @click="updateIsEditBalanceShown" mode="darkSmall">Edit Balance</base-button>
        </div>
        <div class="balance__row">
            <div class="balance__label">Name</div>
            <div class="balance__name">{{ fullName }}</div>
        ...
        <div class="balance__row" v-if="isEditBalanceShown">
            <div class="balance__label">Edit Balance</div>
            <input type="number" v-model="currentBalance" required />
        </div>
        <div class="balance__note" v-if="!ticket?.creatorInfo.balance">*Balance has not been set for this user. Please
            enter.</div>
        <div class="balance__row">
            <div class="balance__label">Amount</div>
